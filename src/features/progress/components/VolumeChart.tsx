...
export function VolumeChart({ data }: { data: SessionPoint[] }) {
  ...
  if (points.length === 0) {
    return <p className="text-muted-foreground text-sm">Log completed sets for this exercise to see volume here.</p>
  }

  const latest = points[points.length - 1].volume
  const best = Math.max(...points.map((point) => point.volume))

  return (
    <div className="space-y-3">
      <div className="flex gap-4">
        <ChartStat label="Latest" value={`${latest.toLocaleString()} kg`} />
        <ChartStat label="Best" value={`${best.toLocaleString()} kg`} />
      </div>
      <ChartContainer>
